...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Première couche : surface0 -> base
    vec3 layer1 = mix(base, mantle, S(-.3, .2, (tuv*Rot(radians(-5.))).x));
    vec3 layer2 = mix(base, mantle, S(-.3, .2, (tuv*Rot(radians(-5.))).x));
    ...
    // vec3 finalColor = mix(terminalColor, finalComp, isBlack);
    //
